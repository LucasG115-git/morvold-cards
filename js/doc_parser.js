 ...
 * Resolves successfully even if fetch fails (e.g. file:// protocol / CORS),
 * so the app still initializes when opened as a local file.
 * @returns {Promise} A promise that resolves when the parsing is complete or fetch failed.
 */
function parse_card_actions() {
    return fetch('js/cards.js')
        ...
        .catch(function () {
            // fetch fails when opened via file:// (CORS). App still needs to init.
        });
}
